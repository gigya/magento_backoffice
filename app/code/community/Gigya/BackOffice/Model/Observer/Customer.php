...
class Gigya_BackOffice_Model_Observer_Customer
{
    public function refreshFromGigya($observer) {
        ...
        $adminSession = Mage::getSingleton('admin/session', array('name' => 'adminhtml'))->isLoggedIn();
        if ($adminSession === true && null === $exec && count(Mage::app()->getRequest()->getPost()) === 0) {
            ...
            if (is_numeric($gigyaAccount)) {
                Mage::log("Error retrieving account from gigya error code was " . $gigyaAccount);
            } else {
